...
immuneValpha <- function(x, ad = bN1, transformation = medcode2){
  ...
  #gpXmtx
   ...
   out <- as.data.frame(thing$table[2, "p-values"])
   colnames(out) = "pval"
   out
}

immuneValpha(use.immune %>% filter(visitno == 9, type == "IgG", antigen == "Con.6.gp120.B"), bN1)
```
...
```{r}
immuneAlphaTable <- immuneAlphaCompiled %>% mutate(qval = p.adjust(pval, method = "BH"))
immuneAlphaTable
```

```{r}
locp <- immuneAlphaCompiled$pval
qvalue(locp)
```
