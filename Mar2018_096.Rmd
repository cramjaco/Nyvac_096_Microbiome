...
```
```{r}
richCI <- bN1 %>% map_df("interval") %>% t %>% as.data.frame() %>% rename(lb = V1, ub = V2) %>% merge(as.data.frame(richEsts), by = "row.names") %>% transform(row.names = Row.names, est = richEsts, Row.names = NULL, richEsts = NULL)
...
```{r}
psN1rich <- psN1
sample_data(psN1rich) <- merge(psN1@sam_data, as.data.frame(richEsts), by = "row.names") %>%
  transform(row.names = Row.names, Row.names = NULL, rich = RichEsts)
```
