...
colnames(gp120mtx) = c("(Intercept)", "predictors")
```
...
Why is qval the same as p-values
If I run it outside of tidyverse, things work as expected
```{r}
immuneAlphaCompiled$`p-values`
"moo"
p.adjust(immuneAlphaCompiled$`p-values`, method = "BH")
```

Inside though
```{r}
immuneAlphaCompiled %>% ungroup %>% mutate(Q2 = p.adjust(`p-values`, method = "BH")) 
```

Example for stack overflow

```{r}
testP <- immuneAlphaCompiled %>%  dplyr::select(p = `p-values`)
testP

testP %>% mutate(q = p.adjust(p))
```
...
conciseImmuneAlphaTable <- immuneAlphaTable %>% 
  dplyr::select(Type = type, Antigen = antigen, Month = month, Coef=Estimates, P=`p-values`, Q = qval)
toAlphaTable <- conciseImmuneAlphaTable %>%
  mutate(
    ...
    Q = cell_spec(format_round(Q, 3), "html",
                         ...
                         background = ifelse(Q < 0.2, 'yellow', '')
    )
  )
...
```{r}
psN1rich %>% sample_data %>% as.data.frame %>% dplyr::select(pub_id, richEst, richL)
```
...
coord_fixed(sqrt(psN2.pcoa$CA$eig[2]/psN2.pcoa$CA$eig[1])) +
viridis::scale_fill_viridis(name = 'richEst', direction = 1) +
#scale_colour_manual(name = 'gp41 Primary', values = c('black', 'grey70')) + 
theme_bw()
psN1richMDSP
```
...
#g <- grid.arrange(wuford_gp41, wuford_gp120, ncol = 2)
g <- cowplot::plot_grid(psN1richMDSP, psN1richP, ncol = 1, labels = c("A", "B"), label_size = 24)
g
